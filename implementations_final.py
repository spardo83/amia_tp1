...
# Pregunta 3: Implementar FasterQDA (VERSIÓN CORREGIDA FINAL)
    ...
    def predict(self, X):
        ...
        for class_idx in range(k_classes):
            ...
            for i in range(n_obs):
                # Corregir: tomar la columna correcta
                x_i = unbiased_x[0, :, i:i+1]  # Shape: (p, 1) - usar unbiased_x[0] y tomar columna i
                # Usar la matriz de covarianza correcta
                inv_cov = self.tensor_inv_cov[class_idx]  # Shape: (p, p)
                quadratic_form = x_i.T @ inv_cov @ x_i
...
# Pregunta 6: Implementar EfficientQDA (VERSIÓN CORREGIDA)
...
# Pregunta 13: Implementar EfficientChol (VERSIÓN CORREGIDA)
